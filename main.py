...
if __name__ == "__main__":
    sys.stderr = open("error.log", "a")  # noqa: PTH123, SIM115
...
def find_dict_keys(node: list | dict, kv: str, *, parent: bool = False) -> Generator:
    if isinstance(node, list):
        for i in node:
            for x in find_dict_keys(i, kv, parent=parent):
                yield x
        ...
        for j in node.values():
            for x in find_dict_keys(j, kv, parent=parent):
                yield x
...
def process_quit(driver: WebDriver) -> None:
    def quit_on_exit(*args) -> None:  # noqa: ARG001
        driver.quit()
...
def get_headless_driver() -> WebDriver:
    ...
    class DriverChoice:
        ...
        driver_options: type[ArgOptions]
        friendly_name: str

    possible_drivers: list[DriverChoice] = [
        DriverChoice(webdriver.Chrome, webdriver.ChromeOptions, "chrome"),
        # DriverChoice(webdriver.Firefox, webdriver.FirefoxOptions, "firefox"),
    ]
    driver = None

    exceptions: list[tuple[str, WebDriverException]] = []
    for driver_choice in possible_drivers:
        try:
            ...
            exceptions.append((driver_choice.friendly_name, e))
            continue

    cls()
    print(
        "This script needs either Chrome or Firefox to be installed and the respective "
        "Web Driver for it to be configured "
        "(usually simplest is by placing it in the folder with the script)"
    )
    print("https://www.browserstack.com/guide/geckodriver-selenium-python")
    for browser, exception in exceptions:
        logger.exception(browser, exc_info=exception)
...
def prompt_mode(order_details: dict, humble_session: WebDriver) -> str:  # noqa: ARG001
    mode = None
...
def valid_steam_key(key: str) -> bool:
    # Steam keys are in the format of AAAAA-BBBBB-CCCCC
    if not isinstance(key, str):
        return False

    key_length, num_parts, part_length = 17, 3, 5
    key_parts = key.split("-")

    correct_length = len(key) == key_length
    correct_parts = len(key_parts) == num_parts
    correct_part_length = all(len(part) == part_length for part in key_parts)

    return correct_length and correct_parts and correct_part_length


def try_recover_cookies(
    cookie_file: str, session: WebDriver | requests.Session
) -> bool:
    try:
        with open(cookie_file, "rb") as f:  # noqa: PTH123
...
def export_cookies(
    cookie_file: str | Path, session: WebDriver | requests.Session
) -> bool:
    try:
        cookies = None
        if type(session) is requests.Session:
        ...
        with open(cookie_file, "wb") as f:  # noqa: PTH123
...
def verify_logins_session(session: WebDriver | requests.Session) -> list[bool]:
    """
    Returns:
        [humble_status, steam_status]
    """
    if type(session) is requests.Session:
        loggedin = session.get(
            STEAM_KEYS_PAGE, allow_redirects=False
        ).status_code not in (301, 302)
...
def humble_login(driver: WebDriver) -> bool:
    ...
    if (
        ...
        and verify_logins_session(driver)[0]
    ):
        return True
    ...
    while not authorized:
        ...
        while (
            "humble_guard_required" in login_json or "two_factor_required" in login_json
        ):
...
def get_choices(humble_session, order_details):
    months = [month for month in order_details if "choice_url" in month["product"]]
    ...
    choices = []
    for month in months:
        if month["choices_remaining"] > 0 or month["product"].get(
            "is_subs_v3_product", False
        ):  # subs v3 products don't advertise choices, need to get them exhaustively
            ...
            if v3:
                ...
                if identifier not in month["choice_data"]["contentChoiceData"]:
                    for key in month["choice_data"]["contentChoiceData"].keys():
                        if (
                            "content_choices"
                            in month["choice_data"]["contentChoiceData"][key]
                        ):
                            identifier = key
...
def _redeem_steam(session, key, quiet=False):
    ...
    r = session.post(
        STEAM_REDEEM_API, data={"product_key": key, "sessionid": session_id}
    )
    blob = r.json()
...
def prompt_skipped(skipped_games):
    ...
    if os.path.exists("skipped.txt"):
        with open("skipped.txt", encoding="utf-8-sig") as file:
...
def prompt_filter_live():
    ...
    while mode not in ["y", "n"]:
        mode = input(
            "You can either see a list of games we think you already own later in a file, or filter them now. Would you like to see them now? [y/n] "
        ).strip()
        if mode in ["y", "n"]:
            return mode
        print("Enter y or n")
...
def redeem_steam_keys(humble_session, humble_keys):
    ...
    noted_keys = [
        ...
        if key["steam_app_id"] not in owned_app_details.keys()
    ]
    skipped_games = {}
    unownedgames = []
    ...
    print(
        f"Filtered out game keys that you already own on Steam; {len(unownedgames)} keys unowned."
    )

    if skipped_games:
        # Skipped games uncertain to be owned by user. Let user choose
        unownedgames = unownedgames + prompt_skipped(skipped_games)
        print(f"{len(unownedgames)} keys will be attempted.")
    ...
    for key in unownedgames:
        ...
        if key["human_name"] in redeemed or (
            key["steam_app_id"] != None and key["steam_app_id"] in redeemed
        ):
...
def export_mode(humble_session, order_details):
    ...
    steam_config = prompt_yes_no(
        "Would you like to sign into Steam to detect ownership on the export data?"
    )
        ...
        if export:
            ...
            if owned_app_details != None and "steam_app_id" in tpk:
                ...
                if not owned:
                    ...
                    owned = (
                        best_match[1] is not None
                        and best_match[1] in owned_app_details.keys()
                    )
...
def choose_games(driver, choice_month_name, identifier, chosen):
    for choice in chosen:
        ...
        if "tpkds" not in choice:
            ...
            if not ("success" in res or not res["success"]):
                logger.error("Error choosing " + choice["title"])
                logger.error(res)
            else:
                logger.info("Chose game " + choice["title"])
    ...
    for month in months:
        ...
        if first:
            redeem_keys = prompt_yes_no(
                "Would you like to auto-redeem these keys after? (Will require Steam login)"
            )
            first = False
        ...
        while not ready:
            ...
            for idx, choice in enumerate(choices):
                ...
                rating_text = ""
                if (
                    "review_text" in choice["user_rating"]
                    and "steam_percent|decimal" in choice["user_rating"]
                ):
            ...
            if redeem_all:
                ...
                print("\nOPTIONS:")
                print(
                    "To choose games, list the indexes separated by commas (e.g. '1' or '1,2,3')"
                )
                print(
                    f"Or type just 'link' to go to the webpage for this month {auto_redeem_note}"
                )
                print("Or just press Enter to move on.")

                user_input = [
                    uinput.strip()
                    for uinput in input().split(",")
                    if uinput.strip() != ""
                ]
            ...
            if len(user_input) == 0:
                ...
                if len(invalid) > 0:
                    ...
                    chosen = [
                        choice
                        for idx, choice in enumerate(choices)
                        if idx + 1 in user_input
                    ]
                    # This weird enumeration is to keep it in original display order

                    if len(chosen) > remaining:
                        print(
                            f"Too many games chosen, you have only {remaining} choices left"
                        )
                        time.sleep(2)
                    else:
                        print("\nGames selected:")
                        ...
                        confirmed = prompt_yes_no(
                            "Please type 'y' to confirm your selection"
                        )
                        if confirmed:
    ...
    if first:
        ...
        if redeem_keys and len(try_redeem_keys) > 0:
            ...
            updated_monthlies = humble_session.execute_async_script(
                get_humble_orders.replace("%optional%", json.dumps(try_redeem_keys))
            )
...
def main():
    ...
    print("Getting order details, please wait")

    order_details: dict = driver.execute_async_script(
        get_humble_orders.replace("%optional%", "")
    )
    ...
    for filter_file in filters:
        try:
            with open(filter_file) as f:
                keycols = f.read()
            filtered_keys = [
                keycol.strip() for keycol in keycols.replace("\n", ",").split(",")
            ]
            steam_keys = [
                key
                for key in steam_keys
                if key.get("redeemed_key_val", False) not in filtered_keys
            ]
        except FileNotFoundError:
            pass
    if len(steam_keys) != original_length:
        print(f"Filtered {original_length - len(steam_keys)} keys from previous runs")
    ...
    if will_reveal_keys:
        ...
        redeem_steam_keys(
            driver, steam_keys if try_already_revealed else unrevealed_keys
        )
    else:
